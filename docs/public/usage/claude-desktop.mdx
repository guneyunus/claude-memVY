...
description: Use Engram memory search in Claude Desktop with MCP tools
icon: desktop
---
...
Claude Desktop can access your Engram memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **Engram is installed** and the worker service is running
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "/Users/YOUR_USERNAME/.claude/plugins/marketplaces/engram/plugin/scripts/mcp-server.cjs"
          ]
        }
      }
    }
    ...
    ```json
    {
      "mcpServers": {
        "mcp-search": {
          "command": "node",
          "args": [
            "C:\\Users\\YOUR_USERNAME\\.claude\\plugins\\marketplaces\\engram\\plugin\\scripts\\mcp-server.cjs"
          ]
        }
      }
    }
...
1. Ensure Engram has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.engram/claude-mem.db`
