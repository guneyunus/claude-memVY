...
Use `<private>` tags to mark content you don't want persisted in Engram's observation database. This gives you fine-grained control over what gets remembered across sessions.
...
- Claude's responses (not captured by Engram)
...
4. **Test it works**: Check `~/.engram/silent.log` if you're unsure whether tags are being stripped
...
2. Check the database to ensure private content is not stored:
   ```bash
   # Check user prompts
   sqlite3 ~/.engram/claude-mem.db "SELECT prompt_text FROM user_prompts ORDER BY created_at_epoch DESC LIMIT 1;"

   # Check observations
   sqlite3 ~/.engram/claude-mem.db "SELECT narrative FROM observations ORDER BY created_at_epoch DESC LIMIT 1;"
   ```
...
2. Check `~/.engram/silent.log` for errors
...
If you see errors in `~/.engram/silent.log`:
```
