...
Instead of fetching all historical data upfront (expensive), Engram uses a progressive disclosure approach:
...
**MCP Server:** Located at `~/.claude/plugins/marketplaces/engram/plugin/scripts/mcp-server.cjs`

**Worker Service:** Express API on port 37777, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.engram/claude-mem.db`
