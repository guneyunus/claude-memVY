---
title: "Installation"
description: "Install Engram plugin for persistent memory across sessions"
---
...
Install and configure Engram with a single command:
...
Install Engram directly from the plugin marketplace inside Claude Code:
...
> **Important:** Engram is published on npm, but running `npm install -g Engram` installs the
> **SDK/library only**. It does **not** register plugin hooks or start the worker service.
> Always install via `npx @guneyunus/engram install` or the `/plugin` commands above.

## System Requirements

- **Node.js**: 20.0.0 or higher
- **Bun** ≥ 1.0 (auto-installed by `npx @guneyunus/engram install` if missing)
...
Dependencies are installed automatically by `npx @guneyunus/engram install` and `npx @guneyunus/engram repair`. Heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens behind a visible installer spinner. The Setup hook only performs a sub-100ms `version-check.js` read of the `.install-version` marker — on mismatch it prints `run: npx @guneyunus/engram repair` to stderr and exits 0, so it never blocks a session. Works cross-platform on Windows, macOS, and Linux.
...
Data is stored in `~/.engram/`:
- Database: `~/.engram/claude-mem.db`
- PID file: `~/.engram/.worker.pid`
- Port file: `~/.engram/.worker.port`
- Logs: `~/.engram/logs/worker-YYYY-MM-DD.log`
...
Upgrades are automatic when updating via the plugin marketplace. After an external upgrade (for example `claude plugin update`), the Setup hook detects a version-marker mismatch and asks you to run `npx @guneyunus/engram repair`, which installs any missing runtime dependencies and refreshes the marker.

See [CHANGELOG](https://github.com/thedotmack/claude-mem/blob/main/CHANGELOG.md) for complete version history.

## Next Steps

- [Getting Started Guide](usage/getting-started) - Learn how Engram works automatically
- [MCP Search Tools](usage/search-tools) - Query your project history
- [Configuration](configuration) - Customize Engram behavior
