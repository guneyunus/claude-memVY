...
- PID file tracking at `~/.engram/worker.pid`
...
Bun is required to run the worker service. If Bun is not installed, `npx @guneyunus/engram install` (and `npx @guneyunus/engram repair`) installs it globally during setup, with a visible clack spinner:
...
Set the Claude model used for compression via environment variable or `~/.engram/settings.json`:
...
- **Discovery**: `GET /api/health` returns the active port; `~/.engram/settings.json` stores the configured value
