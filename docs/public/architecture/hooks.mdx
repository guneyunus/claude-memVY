...
Engram implements a **5-stage hook system** that captures development work across Claude Code sessions. This document provides a complete technical reference for developers implementing this pattern on other platforms.
...
(Runtime setup is handled out-of-band by `npx @guneyunus/engram install` / `npx @guneyunus/engram repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
