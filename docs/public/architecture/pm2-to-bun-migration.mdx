...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing Engram for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
- **Port File**: `~/.engram/.worker.port` (new)
...
- Database file: `~/.engram/claude-mem.db` (same location)
...
`ensureWorkerRunning()` checks if `~/.engram/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.engram/.pm2-migrated` exists
</Step>
...
5. **Marker creation**: `~/.engram/.pm2-migrated` with timestamp
...
New: ~/.engram/logs/worker-YYYY-MM-DD.log
```
...
New: ~/.engram/.worker.pid
     ~/.engram/.worker.port
     ~/.engram/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.engram/claude-mem.db
```
...
**Manual Resolution**:
```bash
rm ~/.engram/.worker.pid
rm ~/.engram/.worker.port
...
cat ~/.engram/.pm2-migrated  # Should exist
