...
description: "Common issues and solutions for Engram"
---
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('Engram-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('Engram-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/engram/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/engram/.install-version
   cat ~/.claude/plugins/marketplaces/engram/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.engram/claude-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/engram/package.json
   ```
...
2. Backup database:
   ```bash
   cp ~/.engram/claude-mem.db ~/.engram/claude-mem.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.engram/claude-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install engram` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/engram/
   ```
...
**Solution**: Check `~/.engram/` exists and is writable.
...
- [Configuration](configuration) - Customize Engram
