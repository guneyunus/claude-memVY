...
description: Complete reference for integrating Engram worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building Engram integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// Register tool
vscode.lm.registerTool('Engram-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('Engram', async (request, context, stream, token) => {
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('Engram-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://Engram.ai">
    Complete Engram documentation
  </Card>
