...
description: "Add persistent memory to Gemini CLI with Engram"
---
...
npx @guneyunus/engram status

# Check hooks are installed — look for Engram entries
...
On session start, you'll see Engram context injected with your recent observations and project history.
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with Engram commands.
...
# View logs
npx @guneyunus/engram logs

# Restart worker
npx @guneyunus/engram restart
```
...
3. Check your AI provider is configured in `~/.engram/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your Engram version may need updating:
...
```bash
npx @guneyunus/engram uninstall
```
