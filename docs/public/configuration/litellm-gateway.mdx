...
description: "Route Engram's Claude Agent SDK worker through LiteLLM while keeping one agentic execution path"
---

# LiteLLM Gateway

Engram can route its background memory agent through a LiteLLM proxy. This lets teams keep Engram's Claude Agent SDK workflow while using LiteLLM for model routing, centralized credentials, usage tracking, budgets, audit logs, and provider failover.
...
- A single organization-level LLM gateway for Engram traffic
- Provider routing without changing Engram source code
- Centralized API keys instead of storing provider keys in each developer's Engram settings
...
Use the native [OpenRouter Provider](../usage/openrouter-provider) or [Gemini Provider](../usage/gemini-provider) instead if you want Engram's REST providers directly and do not need the Claude Agent SDK path.

## Architecture

### One Agent Path

The LiteLLM integration is intentionally small. There is no custom LiteLLM provider, no Python handler, and no OpenAI-compatible server embedded in Engram.

At runtime:

1. The installer or user writes gateway settings to `~/.engram/.env`.
...
7. The SDK response is parsed by the normal Engram observation pipeline.
...
| `src/shared/EnvManager.ts` | Stores credentials in `~/.engram/.env`, blocks shell-leaked auth vars, and injects only explicit Engram credentials |
...
LiteLLM is a gateway for the Claude Agent SDK path, not a fourth Engram provider.
...
Keeping the provider as `claude` matters because the worker should continue to use `ClaudeProvider`, not the native Gemini or OpenRouter REST providers. The gateway URL changes where the SDK sends model traffic; it does not change how Engram manages memory sessions.
...
For Engram, that value goes in `~/.engram/.env`, not your shell, so the background worker uses it consistently across restarts.

### Minimal LiteLLM Example

Create a LiteLLM config that defines the model name Engram will request:
...
In this example, Engram asks the SDK for `claude-haiku-4-5-20251001`, LiteLLM accepts that model alias, and LiteLLM forwards the request to `openai/gpt-4o-mini`.
...
The installer stores provider settings in `~/.engram/settings.json` and gateway credentials in `~/.engram/.env`.

### Option 2: Manual Files

Edit `~/.engram/settings.json`:
...
Engram deliberately does not trust whatever Anthropic credentials happen to be exported in your shell or project `.env` file.

The worker blocks inherited `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, and stale `CLAUDE_CODE_OAUTH_TOKEN` values. It then re-injects only the credentials stored in `~/.engram/.env`.
...
If `ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, or `ANTHROPIC_API_KEY` is present in `~/.engram/.env`, the worker treats that as explicit gateway/API configuration and skips Claude OAuth lookup. This prevents a configured gateway from falling back to `api.anthropic.com`.
...
When a Claude Code session produces transcript events, Engram's worker queues them for observation extraction. In gateway mode the extraction flow is:
...
3. `buildIsolatedEnvWithFreshOAuth()` loads `~/.engram/.env`.
...
8. Engram parses observations, stores them in SQLite, and syncs searchable embeddings.
...
- Engram's worker process
- The Claude Agent SDK subprocess
- Engram's observation XML format
- SQLite storage
- Chroma/vector sync
- Hook installation
- Session resume handling inside Engram

## Verification

Check Engram's worker logs:
...
- `ANTHROPIC_BASE_URL` is in `~/.engram/.env`
...
### Engram still uses Anthropic directly

Check `~/.engram/.env`. Gateway settings must be stored there. Shell exports are not the reliable configuration source for the worker.
...
If LiteLLM uses a master key or virtual key, store it as `ANTHROPIC_AUTH_TOKEN` in `~/.engram/.env`. The Claude Agent SDK sends this value as gateway authorization.

If you previously configured a direct Anthropic API key, remove `ANTHROPIC_API_KEY` from `~/.engram/.env` for gateway mode unless your gateway explicitly expects that variable.
...
Engram's memory worker disables file and shell tools for observation extraction. The LiteLLM gateway is only handling the model call used to compress and summarize memory; it is not a replacement for your interactive Claude Code tool loop.
