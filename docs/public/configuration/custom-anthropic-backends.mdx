...
description: "Point Engram at bridged or self-hosted Anthropic-compatible API endpoints with ANTHROPIC_BASE_URL"
---

# Custom Anthropic-Compatible Backends

When you use the `claude` provider, Engram talks to the Anthropic API through the Claude Agent SDK. By default, the SDK targets the official Anthropic endpoint, but it honors the standard `ANTHROPIC_BASE_URL` environment variable. That means you can route Engram at any Anthropic-protocol-compatible backend — for example a corporate gateway, a regional bridge, or a third-party provider that exposes an Anthropic-shaped API — without changing any Engram source code.
...
Use `ANTHROPIC_BASE_URL` if you need Engram's observation worker to talk to:
...
If your provider only speaks the OpenAI chat-completions protocol, put a gateway such as LiteLLM in front of it and point Engram's Claude Agent SDK path at that gateway. See [LiteLLM Gateway](litellm-gateway) for the full routing model.

## How the Plumbing Works

The flow is intentionally simple:

1. **You write the credential** to `~/.engram/.env`.
...
4. **`ClaudeProvider.startSession()`** spawns the Claude Agent SDK with that isolated env (`src/services/worker/ClaudeProvider.ts:115`). The SDK reads `ANTHROPIC_BASE_URL` natively — Engram does not parse or rewrite it.
...
### Step 1: Edit `~/.engram/.env`

The credentials file is a plain `KEY=VALUE` env file at `~/.engram/.env` (mode `0600`). Add or update the `ANTHROPIC_BASE_URL` line:
...
- **`ANTHROPIC_BASE_URL` from your shell is not inherited.** `ANTHROPIC_API_KEY` is in the BLOCKED_ENV_VARS list (`src/shared/EnvManager.ts:10`) to prevent accidental billing on a shell-leaked key — `ANTHROPIC_BASE_URL` is not blocked, but it must still be set in `~/.engram/.env` for the worker to pick it up reliably across restarts. Do not rely on shell exports.
- **No auto-detection.** If you have already configured `ANTHROPIC_BASE_URL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`, etc. for Claude Code itself, Engram will **not** read those today. Mirror the relevant values into `~/.engram/.env` and `~/.engram/settings.json`.

## Related

- [Configuration](../configuration) — All Engram settings
