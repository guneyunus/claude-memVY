...
The OpenClaw plugin gives Engram persistent memory to agents running on the [OpenClaw](https://openclaw.ai) gateway. It handles three things:

1. **Observation recording** — Captures tool usage from OpenClaw's embedded runner and sends it to the Engram worker for AI processing
...
<Info>
OpenClaw's embedded runner (`pi-embedded`) calls the Anthropic API directly without spawning a `claude` process, so Engram's standard hooks never fire. This plugin bridges that gap by using OpenClaw's event system to capture the same data.
</Info>
...
The observation feed sends a formatted message to your OpenClaw channel every time Engram creates a new observation. Each message includes the observation title and subtitle so you can follow along as your agents work.
...
Add the `observationFeed` block to your Engram plugin config in your OpenClaw gateway configuration:
...
1. **Check the logs** — You should see:
   ```
   [Engram] Observation feed starting — channel: telegram, target: 123456789
   [Engram] Connecting to SSE stream at http://localhost:37777/stream
   [Engram] Connected to SSE stream
   ```

2. **Use the status command** — Run `/claude_mem_feed` in any OpenClaw chat to see:
   ```
   Engram Observation Feed
   Enabled: yes
   Channel: telegram
   Target: 123456789
...
Add `Engram` to your OpenClaw gateway's plugin configuration:
...
<Note>
The Engram worker service must be running on the same machine as the OpenClaw gateway. The plugin communicates with it via HTTP on `localhost:37777`.
</Note>
...
<ParamField body="workerPort" type="number" default={37777}>
  Port for the Engram worker service. Override if your worker runs on a non-default port.
</ParamField>
...
The plugin uses HTTP calls to the already-running Engram worker service rather than spawning subprocesses. This means:
...
Each OpenClaw agent session gets a unique `contentSessionId` (format: `openclaw-<sessionKey>-<timestamp>`) that maps to a Engram session in the worker. The plugin tracks:
