---
title: "Configuration"
description: "Environment variables and settings for Engram"
---
...
Settings are managed in `~/.engram/settings.json`. The file is auto-created with defaults on first run.
...
Gateway credentials live in `~/.engram/.env`, not `settings.json`.
...
`npx @guneyunus/engram install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.engram/settings.json`.
...
- **Production (installed plugin)**: `~/.engram/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `CLAUDE_MEM_DATA_DIR`

```
~/.engram/
├── claude-mem.db           # SQLite database
├── .install-version        # Version marker written by `npx @guneyunus/engram install`/`repair`
...
Engram provides MCP search tools for querying your project history.
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.engram/settings.json`)
- **Auto-Invocation**: Claude recognizes natural language queries about past work

## Version Channel

Engram supports switching between stable and beta versions via the web viewer UI.
...
Engram injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
Settings can be customized in `~/.engram/settings.json`.
...
Control which tools are excluded from observations. Edit `~/.engram/settings.json`:
```json
{
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx @guneyunus/engram install` / `npx @guneyunus/engram repair` outside the session lifecycle.
