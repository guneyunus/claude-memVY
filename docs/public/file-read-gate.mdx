...
description: "How Engram intercepts file reads to save tokens using observation history"
---
...
2. Find the Engram hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of Engram's layered context strategy:
