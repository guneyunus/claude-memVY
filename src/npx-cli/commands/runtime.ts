...
function ensureInstalledOrExit(): void {
  if (!isPluginInstalled()) {
    console.error(pc.red('Engram is not installed.'));
    console.error(`Run: ${pc.bold('npx @guneyunus/engram install')}`);
    process.exit(1);
  }
}
...
function spawnBunWorkerCommand(command: string, extraArgs: string[] = []): void {
  ...
  if (!existsSync(workerScript)) {
    ...
    console.error('The installation may be corrupted. Try: npx @guneyunus/engram install');
    process.exit(1);
  }
  ...
  const child = spawnHidden(bunPath, args, {
    ...
    // re-read from ~/.engram/.env at SDK spawn time (#2357 / #2375).
...
export async function runSearchCommand(queryParts: string[]): Promise<void> {
  ...
  if (!query) {
    console.error(pc.red('Usage: npx @guneyunus/engram search <query>'));
    process.exit(1);
  }
  ...
  try {
    ...
    if (cause?.code === 'ECONNREFUSED' || message.includes('ECONNREFUSED')) {
      ...
      console.error(`Start it with: ${pc.bold('npx @guneyunus/engram start')}`);
      process.exit(1);
    }
  ...
  if (!response.ok) {
    if (response.status === 404) {
      ...
      console.error(`Try: ${pc.bold('npx @guneyunus/engram start')}`);
      process.exit(1);
    }
