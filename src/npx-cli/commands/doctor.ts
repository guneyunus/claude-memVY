/**
 * `npx @guneyunus/engram doctor` — a minimal diagnostic that probes every layer an
...
export async function runDoctorCommand(): Promise<void> {
  ...
  checks.push({
    ...
    detail: installed ? marketplaceDirectory() : 'run `npx @guneyunus/engram install`',
    required: true,
  });
  ...
  checks.push({
    ...
    detail: depsPresent ? 'node_modules present' : 'missing — run `npx @guneyunus/engram repair`',
    required: installed,
  });
  ...
  let workerDetail = `no response on port ${workerPort} — start with \`npx @guneyunus/engram start\``;
  try {
