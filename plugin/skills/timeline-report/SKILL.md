...
The Engram worker must be running. The project must have Engram observations recorded.
...
- **Worker not running:** "The Engram worker is not responding on port ${WORKER_PORT}. Start it with your usual method or check `ps aux | grep worker-service`."
