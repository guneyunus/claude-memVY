---
name: how-it-works
description: Explain how engram captures observations, when memory injection kicks in, and where data lives. Use when the user asks "how does engram work?" or "what is this thing doing?".
---
...
Everything stays in ~/.engram on this machine.

Nothing leaves your machine except calls to whichever AI provider you configured for compression (Claude / OpenRouter / Gemini). The SQLite database, vector index, logs, and settings all live under that directory and are removed cleanly on `npx @guneyunus/engram uninstall`.
